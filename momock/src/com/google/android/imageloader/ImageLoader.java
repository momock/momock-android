...
public class ImageLoader {
    ...
    protected final ContentHandler mBitmapContentHandler;

    protected final ContentHandler mPrefetchContentHandler;

    protected final URLStreamHandlerFactory mURLStreamHandlerFactory;

    protected final HashMap<String, URLStreamHandler> mStreamHandlers;

    protected final LinkedList<ImageRequest> mRequests;
     ...
     */
    protected final Map<String, Bitmap> mBitmaps;

    /**
     * Recent errors encountered when loading bitmaps.
     */
    protected final Map<String, ImageError> mErrors;
     ...
     */
    protected final Map<ImageView, String> mImageViewBinding;

    /**
     * The maximum number of active tasks.
     */
    protected final int mMaxTaskCount;

    /**
     * The current number of active tasks.
     */
    protected int mActiveTaskCount;
    ...
    protected void enqueueRequest(ImageRequest request) {
        mRequests.add(request);
        flushRequests();
    }

    protected void insertRequestAtFrontOfQueue(ImageRequest request) {
    ...
    protected void putError(String url, ImageError error) {
    ...
    protected ImageError getError(String url) {
     ...
     */
    protected boolean hasError(String url) {
        return getError(url) != null;
    }

    protected class ImageRequest {

        protected final ImageCallback mCallback;

        protected final String mUrl;

        protected final boolean mLoadBitmap;
        ...
        protected Bitmap loadImage(URL url) throws IOException {
        ...
        public boolean execute() {
            try {
                ...
                if (mLoadBitmap) {
                    ...
                    mBitmap = null;
                    return true; // jacky : we still need to trigger the callback !
                }
            } catch (IOException e) {
    ...
    protected final class ImageViewCallback implements ImageCallback {
    ...
    protected static final class BaseAdapterCallback implements ImageCallback {
        protected final WeakReference<BaseAdapter> mAdapter;
    ...
    protected static final class BaseExpandableListAdapterCallback implements ImageCallback {

        protected final WeakReference<BaseExpandableListAdapter> mAdapter;
    ...
    protected class ImageTask extends AsyncTask<ImageRequest, ImageRequest, Void> {
    ...
    protected static class ImageError {
        protected static final int TIMEOUT = 2 * 60 * 1000; // Two minutes
